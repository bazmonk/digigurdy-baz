...
//    CHOOSE WHICH SCREEN YOU INTEND TO USE 
//#include <Adafruit_SSD1306.h> // Note used in this version. This is used if you have an Adafruit OLED screen that requires the SSD1306 driver. Comment out if using the blue non-Adafruit screens
#include <Adafruit_SH1106.h>   //<----------------Uncomment this for the Blue non-Adafruit screens ---------------------------------------------------------------------
//Must use the Wonho-maker version of the Adafruit_SH1106 library for it to work correctly
...
#define OLED_CLK 10 // Also known as SCK sometime
#define OLED_DC 11
#define OLED_CS 12
#define OLED_RESET 13
Adafruit_SH1106 display(OLED_MOSI, OLED_CLK, OLED_DC, OLED_RESET, OLED_CS);     // <--------UNCOMMENT IF USING THE BLUE NON-ADAFRUIT SCREENS---------------------------------------------------------------------------------------
// Changed to this for our SH1106 driver screen

//Adafruit_SSD1306 display(OLED_MOSI, OLED_CLK, OLED_DC, OLED_RESET, OLED_CS); //  <---------UNCOMMENT IF USING THE ADAFRUIT WHITE OLED SCREENS----------------------------------------------------------------------------------
...
void setup() {
  ...
  //  (neat!)
  //display.begin(SSD1306_SWITCHCAPVCC); // <------------------ UNCOMMENT IF USING THE ADAFRUIT WHITE OLED SCREENS -------------------------
  display.begin(SH1106_SWITCHCAPVCC);  //   <------------------ UNCOMMENT IF USING THE NON-ADAFRUIT BLUE OLED SCREENS ----------------------
  ...
  // Work out Standard Deviation of lots of
  // samples of analog Pin 2 i.e. the one the crank gearmotor is connected to
  // If gearmotor of the crank module is connected the
  ...
  delay(2000);
  display.clearDisplay();
  // XXXXXXXXXXXXX End of work out SD XXXXXXXXXXXXXXXXX
  ...
  // XXXXXXXXXXXXX  KEEP PREVIOUS DISPLAY AND TUNINGS   YES OR NO  XXXXXXXX
  display.clearDisplay();
  display.setTextSize(1);
  display.setCursor(0, 0);
  ...
  if (settings == 2) { // i.e. we DO want to enter menus to change the settings
    // Do RE MI or original display selection
  ...
  // RETRIEVE SAVED TUNING OPTION AND DISPLAY STYLE IF APPROPRIATE
  ...
  if (settings == 1) {
    ...
    buzzrootkey = trompmidi; // Set the buzz pitch to match the trompette value  <------------------------------ May alter this so of trompmidi too low, we make the buzz one octave above
...
void loop() {
  ...
  // READ THE GAMING CONSOLE FIRE BUTTON, ONE PRESS TO START CRANK PRESS AGAIN TO STOP IT
  cr = cr + 1;
  if (cr == 200) { // Looks at big arcade gaming button status every 200 program
    ...
    // END OF READ CRANK FIRE BUTTON STATUS
  ...
  if (aold != a &&
    ...
    // XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX  
    // Melody string section start, both variants as regards the drones
    // XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
  ...
  */
  // End of look at crank situation
...
void clearallMIDI() { // Precautionary step that turns everything off that might
 ...
 // New way of doing this with a control change
///https://www.presetpatch.com/midi-cc-list.aspx

// "123 All MIDI Notes OFF CC123 is a switch used to tell an instrument to turn all active notes off. 
//  This may be useful if a note is hung in the on position. Values of 0-63 indicate OFF. Values 26-127 indicate ON."
 
 usbMIDI.sendControlChange(123,1,1); 
 usbMIDI.sendControlChange(123,1,2); 
 usbMIDI.sendControlChange(123,1,3); 
 usbMIDI.sendControlChange(123,1,4);  
 usbMIDI.sendControlChange(123,1,5); 
 usbMIDI.sendControlChange(123,1,6); 
...
*/
  ...
  buzzrootkey =
      trompmidi; // Set the buzz pitch to match the new trompette value <------------------------------------May need altering if trompmidi is too low we set buzzrootkey one octave higher else sounds bad
